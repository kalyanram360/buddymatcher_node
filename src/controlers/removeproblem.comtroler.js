...
const removeProblemFromDB = async (title) => {
  if (!title) return;
  ...
  try {
    ...
    if (existingProblem.online_now <= 0) {
      await Problems.deleteOne({ title });
      console.log(`Problem ${title} deleted as online_now is 0`);
    }
  } catch (error) {
    console.error("Error in removeProblemFromDB:", error);
  }
};

export default removeProblemFromDB;
